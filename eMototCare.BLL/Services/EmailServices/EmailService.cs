...
namespace eMototCare.BLL.Services.EmailServices
{
    ...
    "<a href=\"" + verifyUrl + "\" style=\"color: #1a0dab; text-decoration: underline;\">Kích hoạt tài khoản</a></p>" +
    ...
    "</body>";
        ...
        public async Task SendAccountInfo(string to, string subject, string verifyUrl)
        {
            var fromName = Environment.GetEnvironmentVariable("MailSettings__FromName")
                      ?? _settings.FromName;
            var fromEmail = Environment.GetEnvironmentVariable("MailSettings__FromEmail")
            ...
            "<h1 style=\"font-size: 24px; margin: 0; padding: 0;\">Kích hoạt tài khoản nhân viên</h1>" +
            "<p style=\"font-size: 16px; line-height: 1.5;\">Chào mừng bạn đến với hệ thống eMotoCare!</p>" +
            "<p style=\"font-size: 16px; line-height: 1.5;\">Thông tin tài khoản nhân viên của bạn đã được tạo:</p>" +

            "<div style=\"background-color: #f9f9f9; padding: 15px; border-radius: 6px; margin-top: 10px; text-align: left; font-size: 15px;\">" +
            "<strong>Email đăng nhập:</strong> " + to + "<br/>" +
            "<strong>Mật khẩu mặc định:</strong> 12345678<br/>" +
            "</div>" +
            ...
            "<p style=\"font-size: 14px; color: #d32f2f; margin-top: 20px; font-weight: bold;\">" +
            "Lưu ý: Sau khi kích hoạt tài khoản, vui lòng đổi mật khẩu ngay để tránh bị khoá tài khoản hoặc truy cập trái phép!" +
            "</p>" +
