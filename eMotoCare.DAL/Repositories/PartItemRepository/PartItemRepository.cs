...
namespace eMotoCare.DAL.Repositories.PartItemRepository
{
    ...
    {
        public PartItemRepository(ApplicationDbContext context)
            : base(context) { }
        ...
        )
        {
            ...
            if (serviceCenterId.HasValue)
                q = q.Where(x =>
                    x.ServiceCenterInventory != null
                    && x.ServiceCenterInventory.ServiceCenterId == serviceCenterId.Value
                );
        ...
        public Task<PartItem?> GetByIdAsync(Guid id) =>
            _context
                .PartItems.Include(x => x.ServiceCenterInventory)
        ...
        public async Task<List<PartItem>> GetByServiceCenterIdAsync(Guid serviceCenterId)
        {
            return await _context
                .PartItems.Include(p => p.ServiceCenterInventory)
                .Include(p => p.Part)
                .Where(p => p.ServiceCenterInventory.ServiceCenterId == serviceCenterId)
                .ToListAsync();
        }

        public async Task<List<PartItem>> GetAvailablePartItemsByPartIdAsync(
            Guid partId,
            Guid serviceCenterId
        )
        {
